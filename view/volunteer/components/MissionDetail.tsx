...
export const MissionDetail: React.FC<MissionDetailProps> = ({ task, onBack, onAccept, volunteerName = "Relawan" }) => {
    ...
    const handleRoute = (location?: { lat: number, lng: number, address: string }) => {
        let destination = "";
        
        if (location?.address && location.address !== "Lokasi tidak tersedia") {
            ...
            <div className="p-6 pb-40 space-y-6 max-w-2xl mx-auto">
                ...
                <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <Button
                            onClick={() => handleRoute(task.donorLocation)}
                        ...
                        >
                            <span className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5" /> Rute Jemput</span>
                        </Button>
                        <Button
                            onClick={() => handleRoute(task.receiverLocation)}
                            className="h-12 rounded-xl text-[10px] font-black uppercase tracking-widest bg-green-600 hover:bg-green-500 text-white shadow-lg shadow-green-500/20 border-0 flex flex-col items-center justify-center gap-0.5 leading-none"
                        >
                            <span className="flex items-center gap-1.5"><Navigation className="w-3.5 h-3.5" /> Rute Antar</span>
                        </Button>
                    </div>
                ...
                <div className="max-w-2xl mx-auto flex gap-4">
                    {task.status === 'available' ? (
                        ...
                        <div className="flex gap-3 w-full">
                            <Button 
                                onClick={() => handleRoute(task.donorLocation)}
                                className="flex-1 h-14 rounded-2xl text-xs font-black uppercase tracking-widest bg-orange-600 hover:bg-orange-500 text-white shadow-lg border-0"
                            >
                                <Navigation className="w-4 h-4 mr-2" /> RUTE JEMPUT
                            </Button>
                            <Button 
                                onClick={() => handleRoute(task.receiverLocation)}
                                className="flex-1 h-14 rounded-2xl text-xs font-black uppercase tracking-widest bg-green-600 hover:bg-green-500 text-white shadow-lg border-0"
                            >
                                <Navigation className="w-4 h-4 mr-2" /> RUTE ANTAR
                            </Button>
                        </div>
                    )}
                </div>
