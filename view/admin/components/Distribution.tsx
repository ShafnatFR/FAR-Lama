...
import { DistributionTask, ClaimHistoryItem, UserData, FoodItem } from '../../../types';
...
export const Distribution: React.FC<DistributionProps> = ({ claims = [], users = [], inventory = [] }) => {
  ...
  useEffect(() => {
      const tasks: ExtendedDistributionTask[] = claims
        ...
        .map(c => {
            ...
            const donorPhone = providerUser?.phone || '6285215376975'; 
            const donorLocation = c.location?.address || 'Lokasi Donatur';

            // 2. Lookup Receiver Info
            const receiverUser = users.find(u => String(u.id).trim() === String(c.receiverId).trim());
            const receiverPhone = c.receiverPhone || receiverUser?.phone || '6285215376975';
            const receiverLocation = (c as any).receiverLocation?.address || 'Lokasi Penerima';
