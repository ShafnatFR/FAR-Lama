...
export const getDateTimeParts = (dateStr?: string, timeStr?: string) => {
  ...
  if (!targetDate && (timeStr || dateStr)) {
    const val = timeStr || dateStr;
...
export const formatDateTime = (dateStr?: string, timeStr?: string): string => {
  const parts = getDateTimeParts(dateStr, timeStr);
